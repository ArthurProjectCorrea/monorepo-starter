...
This file defines the mandatory workflow for Copilot Chat (or any AI agent) to automate, document, and submit issues in this monorepo. Follow these steps exactly to ensure every issue is actionable, project-specific, and review-ready.
...
4. **Template Selection and Pre-filling**
   - Locate the appropriate issue template in `.github/ISSUE_TEMPLATE` based on the identified type.
   - Copy the template content **starting only after the YAML front matter** (everything after the second `---` block).
   - Pre-fill all markdown fields with relevant information from the requirement, bug, or context.
   - Never include the YAML front matter (`--- ... ---`) in the issue body sent to GitHub, as this breaks rendering and readability.
   - Save the filled template as a new file in `.github/temp/`, using a clear and unique name (e.g., `feature_request_api.md`, `bug_report_web.md`).
   - Example extraction:
     - Original template:
...
6. **Issue Submission to GitHub**
   ...
   - **Important:** Always remove the YAML front matter (the block between `---` at the top of the file) before submission. Use only the markdown content (starting from the first heading, such as `## 🛠️ Branch Name`) as the issue body to ensure correct formatting on GitHub.
   - Set the project, title, and labels correctly according to the template and context.
...
- Always use clear, technical English and provide actionable information in the pre-filled templates.
